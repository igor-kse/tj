...
<%--@elvariable id="meal" type="ru.javaops.topjava.dto.MealTo"--%>
<%--@elvariable id="now"  type="java.time.LocalDateTime"--%>

<c:set var="editing" value="${meal != null}"/>
<c:choose>
    <c:when test="${editing}"><c:set var="dateTime" value='${d:format(meal.dateTime(), "yyyy-MM-dd\'T\'HH:mm")}' /></c:when>
...
<form action="${pageContext.request.contextPath}/meals" method="post">
    <input type="hidden" name="id" value="${editing ? meal.id() : ''}"/>
    <p>
    ...
    <p>
        ...
        <input type="text" id="description" name="description" value="${editing ? meal.description() : ''}" required/>
    </p>
    <p>
        <label for="calories">Калории:</label><br/>
        <input type="number" id="calories" name="calories" min="1" value="${editing ? meal.calories() : ''}" required/>
    </p>
    <p>
